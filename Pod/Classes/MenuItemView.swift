...
public class MenuItemView: UIView {
    ...
    internal func focusLabel(selected: Bool) {
        ...
        if let menuView = self.superview {
            labelSize = calculateLableSize(menuView.frame.size)
        } else {
            labelSize = calculateLableSize()
        }
        widthLabelConstraint.constant = labelSize.width
    }
    ...
    var labelTextSize:CGSize {
        return NSString(string: title).boundingRectWithSize(CGSizeMake(CGFloat.max, CGFloat.max), options: NSStringDrawingOptions.UsesLineFragmentOrigin, attributes: [NSFontAttributeName: titleLabel.font], context: nil).size
    }
